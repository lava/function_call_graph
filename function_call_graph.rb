  ...
  def grok
    ...
    IO.readlines(@file).each do |line|
      ...
      if f[1] == "->" or f[1] == "=>" then
        ...
        # puts "#{newfunc.name} #{newfunc.stime}"
    ...
    @stime = self.min{|a,b| a.stime <=> b.stime}.stime
...
class FunctionCallGraphSVG
  ...
  def initialize(file, width, height = 0)
    ...
    # I still havent figure out how to force the browser to
    # display the individual svg's below each other - as a
    # workaround, we scale the svg width so it will be always
    # wide enough to overflow the screen.
    @svg = SVG::Svg.new("width" => width*10,
      "height"  => @height,
...
function clearfunc() {funcele.nodeValue = ' ';}
  ...
  def draw
    xscale = 1.0 * (@width - PADDING_LEFT - PADDING_RIGHT)/(@fg.etime - @fg.stime)
    g = SVG::G.new("transform" => "translate(0, 0)")
    @svg.add(g)

    @fg.each do |node|
      x1 = (node.stime - @fg.stime)*xscale + PADDING_LEFT
      y1 = (FUNCTION_HEIGHT + BLOCK_PADDING_TOP) * node.level
      w = (node.etime - node.stime ) * xscale
      if w < 0.5
        # The original script skipped these since the width is less than 1/2 pixels,
        # but with infinite zoom due to svg and browsers powerful enough to render tens
        # of thousands of rectangles, there seems to be no need anymore.
        STDERR.puts "Including #{node.name} with w = #{w} = #{xscale} * (#{node.etime} - #{node.stime})"
        #next
      end
...
def main
  ...
  ARGV.each do |inputfile|
    f = FunctionCallGraphSVG.new(inputfile, 600)
    f.draw
    puts f.to_s
  end
  puts "</body></html>"
